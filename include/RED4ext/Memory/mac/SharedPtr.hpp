...
struct RefCnt
{
    ...
    void IncRef()
    {
        __atomic_add_fetch(&strongRefs, 1, __ATOMIC_RELAXED);
    }

    // Returns true if the strong refs count has been successfully incremented.
    bool IncRefIfNotZero()
    {
        uint32_t uses = __atomic_load_n(&strongRefs, __ATOMIC_RELAXED);
        while (uses != 0)
        {
            if (__atomic_compare_exchange_n(&strongRefs, &uses, uses + 1, true, __ATOMIC_RELAXED, __ATOMIC_RELAXED))
            {
                return true;
            }
        }
    ...
    bool DecRef()
    {
        return __atomic_sub_fetch(&strongRefs, 1, __ATOMIC_ACQ_REL) == 0;
    }

    void IncWeakRef()
    {
        __atomic_add_fetch(&weakRefs, 1, __ATOMIC_RELAXED);
    }
...
public:
    ...
    {
        return refCount ? __atomic_load_n(&refCount->strongRefs, __ATOMIC_RELAXED) : 0;
    }
