...
{
    auto address = reinterpret_cast<uintptr_t>(RED4ext::RelocBase::GetImageBase()) + aRVAAddress;
...
RED4EXT_INLINE bool RED4ext::TweakDB::UpdateRecord(gamedataTweakDBRecord* aRecord)
{
    ...
    static UniversalRelocFunc<CreateTDBRecord_t> CreateTDBRecord(Detail::Addresses::TweakDB_CreateRecord);
    ...
    struct FakeAllocator : Memory::IAllocator
    {
        ...
        virtual Memory::AllocationResult AllocAligned(uint64_t aSize, uint32_t aAlignment) const override
        {
            ...
            size_t alignedSize = (aSize + aAlignment - 1) & ~(aAlignment - 1);
            result.memory = aligned_alloc(aAlignment, alignedSize);
            result.size = aSize;
            return result;
        }
...
RED4EXT_INLINE int32_t RED4ext::TweakDB::CreateFlatValue(const CStackType& aStackType)
{
    ...
    uintptr_t flatValueSize =
        RED4ext::AlignUp(static_cast<uintptr_t>(8ull /* vftable */ + aStackType.type->GetSize()), flatAlignment);
...
{
    using Get_t = TweakDB* (*)();
    static UniversalRelocFunc<Get_t> func(Detail::Addresses::TweakDB_Get);
    return func();
}
